...
class FactorAnalysisModel(TensorusModel):
    """Factor Analysis using ``sklearn.decomposition.FactorAnalysis``."""

    def __init__(self, n_components: Optional[int] = None, **kwargs) -> None:
        self.n_components = n_components
        self.kwargs = kwargs
        self.model: Optional[FactorAnalysis] = None
    ...
    def fit(self, X: Any, y: Any = None) -> None:
        ...
        self.model = FactorAnalysis(n_components=self.n_components, **self.kwargs)
        self.model.fit(X_np)
